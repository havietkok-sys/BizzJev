...
- Validation-only tolerances (response structure checks, never used in policy comparisons): distribution probabilities must sum to `1` within `0.00001` (`DistributionSumTolerance`), and the reported Score must agree with its probability-weighted distribution within `0.00001` (`ScoreAgreementTolerance`).
