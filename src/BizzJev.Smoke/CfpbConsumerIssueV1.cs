...
internal static class CfpbConsumerIssueV1
{
    private const string TestHash = "97a79e10a513bd5737a350e6b33024b1087ea9657db1f85f17c1c52aef516706";
    private const string JudgmentHash = "f4bd1e56a59db4e1f46d3cc8450c604a64f89249ade1629bfbe49aef32acbca6";
    ...
    internal static async Task<int> Run(bool checkOnly)
    {
        ...
        var definition = File.ReadAllBytes("src/BizzJev.Smoke/cfpb-consumer-issue-v1.json");
        if (!Convert.ToHexString(SHA256.HashData(definition)).Equals(JudgmentHash, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Frozen judgment SHA-256 mismatch. No requests sent.");
        using var questions = JsonDocument.Parse(definition);
        var labels = questions.RootElement.GetProperty("consumerSelectedIssue").GetProperty("criteria").EnumerateObject().Select(p => p.Name).ToArray();
        ...
        if (rows.Count != 810 || rows.Select(r => r.Id).Distinct().Count() != 810
            ...
            || labels.Length != 4 || !labels.ToHashSet().SetEquals(expectedPerLabel.Keys)
        ...
        if (checkOnly)
        {
            ...
            Console.WriteLine("Consumer-issue offline checks passed. No requests sent.");
            return 0;
        }
        ...
        var directory = "data/results/cfpb-consumer-issue-v1-" + DateTime.UtcNow.ToString("yyyyMMddTHHmmssfffZ");
        ...
        {
            name = "CFPB Consumer-Selected Issue Prediction v1",
            ...
            for (var attempt = 1; attempt <= MaxAttempts; attempt++)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        var answer = root.GetProperty("answers").GetProperty("consumerSelectedIssue");
