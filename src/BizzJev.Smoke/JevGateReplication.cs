...
internal static class JevGateReplication
{
    private const string RunInputHash = "0726d6e7cbad2d56e9016a6fd9ea57680d6a7b9c0406c439e595f12f5d6a4fcd";
    ...
    internal static async Task<int> Run(bool checkOnly)
    {
        ...
        var inputBytes = File.ReadAllBytes("data/results/jev-semantic-gate-independent-replication/run_input.csv");
        ...
        var gatesBytes = File.ReadAllBytes("data/results/jev-semantic-gate-independent-replication/gate_definitions.json");
        ...
        if (rows.Count != 162 || rows.Any(r => string.IsNullOrWhiteSpace(r.Narrative)))
            throw new InvalidDataException("Expected 162 non-empty narratives.");
        ...
        var outDir = "data/results/jev-semantic-gate-independent-replication";
